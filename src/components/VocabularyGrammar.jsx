...
import grammarData from '../data/vocabulary_grammar.json';
...
export default function VocabularyGrammar() {
  ...
  const [startId, setStartId] = useState(101);
  const [endId, setEndId] = useState(200);
  ...
  const handleStartQuiz = () => {
    let filtered = grammarData.filter(q => q.id >= startId && q.id <= endId);
  ...
  if (step === 'setup') {
    return (
      <div className="fade-in" style={{ padding: '40px 20px', maxWidth: '500px', margin: '40px auto', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)' }}>
        <h2 style={{ textAlign: 'center', color: '#1e293b', marginBottom: '30px' }}>⚙️ Thiết lập bài thi</h2>
        <p style={{ textAlign: 'center', color: '#64748b', marginBottom: '20px' }}>
          Bài thi có từ câu 1 đến câu 200.
        </p>
        ...
        <style>{`
          ...
          .btn-primary { background-color: #4f46e5; color: white; border: none; borderRadius: 8px; cursor: pointer; transition: all 0.2s; border-radius: 8px; }
  ...
  if (step === 'result') {
    ...
    return (
      <div className="fade-in" style={{ padding: '40px', maxWidth: '500px', margin: '40px auto', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)', textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#1e293b', marginTop: '10px' }}>Hoàn thành bài thi!</h2>
    ...
    <div className="fade-in" style={{ display: 'flex', gap: '30px', maxWidth: '1100px', margin: '40px auto', padding: '0 20px', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          {quizQuestions.map((q, index) => {
            ...
            if (qState && qState.firstAttemptResult === 'correct') {
              boxBgColor = '#d1fae5'; // Xanh lá nhạt
              boxTextColor = '#059669'; // Xanh lá đậm
            } else if (qState && qState.firstAttemptResult === 'wrong') {
              boxBgColor = '#ffe4e6'; // Đỏ nhạt
              boxTextColor = '#e11d48'; // Đỏ đậm
            }

            if (currentIndex === index) {
              boxBorder = '2px solid #4f46e5'; // Viền xanh dương cho câu hiện tại
              boxBgColor = '#e0e7ff';
              boxTextColor = '#4f46e5';
            }
      ...
      <div style={{ flex: '3', backgroundColor: '#fff', padding: '40px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '2px solid #f1f5f9', paddingBottom: '15px' }}>
          <h3 style={{ margin: 0, color: '#475569' }}>Question {currentIndex + 1} <span style={{ color: '#94a3b8', fontSize: '16px' }}>/ {quizQuestions.length}</span></h3>
        ...
        <p style={{ fontSize: '22px', fontWeight: '600', color: '#0f172a', lineHeight: '1.6', marginBottom: '30px' }}>
          {currentQ.question}
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {currentQ.optionsArray.map((opt, index) => {
            ...
            // Cập nhật nhãn hiển thị A, B, C, D theo thứ tự vòng lặp
...
// Inline style cho các ô input số lượng
const inputStyle = {
