...
import clozeData from '../data/cloze_text.json';
...
export default function ClozeText() {
  ...
  const handleStartQuiz = () => {
    ...
    let formatted = clozeData.map(passage => {
      let formattedQuestions = passage.questions.map(q => {
        ...
        // Đảo thứ tự đáp án nếu được chọn
  ...
  if (step === 'setup') {
    return (
      <div className="fade-in" style={{ padding: '40px 20px', maxWidth: '500px', margin: '40px auto', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)' }}>
        <h2 style={{ textAlign: 'center', color: '#1e293b', marginBottom: '20px' }}>🧩 Bài thi Cloze Text</h2>
        <p style={{ textAlign: 'center', color: '#64748b', marginBottom: '30px' }}>
          Bài thi gồm <strong>{clozeData.length}</strong> bài điền từ. Gồm bài 4, 7, 15, 17.
        </p>
  ...
  if (step === 'result') {
    ...
    return (
      <div className="fade-in" style={{ padding: '40px', maxWidth: '500px', margin: '40px auto', backgroundColor: '#fff', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)', textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#1e293b', marginTop: '10px' }}>Hoàn thành phần Cloze Text!</h2>
  ...
  if (isLastQuestionInPassage && !isLastPassage) nextButtonText = "Bài tiếp theo ⏭️";
  ...
  return (
    <div className="fade-in" style={{ maxWidth: '1200px', margin: '20px auto', padding: '0 20px' }}>
      ...
      <div style={{ backgroundColor: '#fff', padding: '15px 25px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0, color: '#1e293b' }}>Bài điền từ: {currentPassageIndex + 1} / {formattedData.length}</h3>
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'stretch' }}>
        ...
        <div className="custom-scrollbar" style={{ flex: '1.2', backgroundColor: '#fff', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', height: '70vh', overflowY: 'auto' }}>
          ...
          <div style={{ color: '#334155', fontSize: '18px', lineHeight: '2.0', whiteSpace: 'pre-line', textAlign: 'justify' }}>
        ...
        <div style={{ flex: '1', display: 'flex', flexDirection: 'column', backgroundColor: '#fff', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', height: '70vh' }}>
          ...
          <div className="custom-scrollbar" style={{ flex: '1', overflowY: 'auto', paddingRight: '10px' }}>
            <p style={{ fontSize: '18px', fontWeight: 'bold', color: '#0f172a', marginBottom: '20px' }}>
              Chọn từ thích hợp điền vào chỗ trống ({currentQuestionIndex + 1}):
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {currentQ.optionsArray.map((opt, index) => {
                ...
                // Cố định hiển thị A, B, C, D bất kể đáp án bị xáo trộn thế nào
          ...
          <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'space-between', paddingTop: '20px', borderTop: '2px solid #f1f5f9' }}>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button 
                ...
                title="Quay lại Câu trước đó trong bài này"
              >
                ← Câu trước
              </button>
            </div>
